...
2020 年 8 月份，游戏线业务经过调整后，开始接触 A 项目，该项目是 2017 年创建，为游戏中心、商店、联运等平台提供内容服务，经过三年的迭代以及不同开发人员的维护。接触几个月，在开发维护的过程中碰到诸多挑战。
...
由于经过长时间的迭代和更新，加上开发同学换了一波又一波，导致代码风格、架构设计不统一，维护成本逐步上升。除此之外，A 项目与其它很多项目存在公共业务逻辑，像游戏卡片、评论互动能力等等。

基础能力和中间层的长期缺乏，导致项目间以及项目内存在大量冗余，无形中为后续需求迭代和开发增加了大量的负担，同一个功能要在不同项目中开发很多遍。

![](./img/card.png)

就拿上面游戏卡片来说，在**A**、**B**、**C**、**D**等等项目中均有实现。当我们需要增加预约卡片时，我们需要修改**A 项目**增加预约卡片、需要修改**B 项目**增加预约卡片、需要修改**C 项目**增加预约卡片、需要修改**D 项目**增加预约卡片等等，这还不包括后续可能还有项目中会有游戏卡片需求，可见维护成本成倍增加。
...
在**E**项目中，有需求要穿插游戏卡片，按照以往的做法，把游戏卡片的逻辑拷贝到新项目中就好了，开发一时爽，维护累够呛！再不对游戏卡片动手，就又给自己掘一个坟墓。
...
我们选取了**A**项目中的游戏卡片，抽离出来单独部署成微前端服务，将之前的卡片逻辑按照**横向分类、纵向分层**的思想，全新重构了。之前将下载卡片、预约卡片、预下载卡片视图层耦合在一起，导致不同种类的卡片业务逻辑也耦合在一起，维护困难。除此之外，也没有数据层和适配层。重构之后的示意图如下：
...
const MICRO_APP_URL =
  process.env.NODE_ENV === 'dev'
    ? '//xxx.vivo.com.cn:7104/game-card/'
    : process.env.NODE_ENV === 'test'
    ? 'https://xxx-test.vivo.com.cn/game-card/'
    : process.env.NODE_ENV === 'pre'
    ? 'https://xxx-pre.vivo.com.cn/game-card/'
    : 'https://xxx.vivo.com.cn/game-card/'
...
如下所示，静态资源设置 publicPath 配置，值为不同环境下的地址，譬如 开发环境就为 `http://localhost:7104/game-card/`，测试环境可能就是 `https://xxx-test.vivo.com.cn/game-card/`，依次类推设置
