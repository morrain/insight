...
export function initGlobalState() {
  ...
  if (state === globalState) {
    console.warn('[insight] state has not changed！');
  } else {
  ...
  return {
    ...
    onGlobalStateChange: function onGlobalStateChange(callback, fireImmediately) {
      if (!(callback instanceof Function)) {
        console.error('[insight] callback must be function!');
        return;
      }

      if (deps[id]) {
        console.warn("[insight] '".concat(id, "' global listener already exists before this, new listener will overwrite it."));
      }
    ...
    setGlobalState: function setGlobalState() {
      ...
      globalState = _cloneDeep(Object.keys(state).reduce(function (_globalState, changeKey) {
        ...
        console.warn("[insight] '".concat(changeKey, "' not declared when init state\uFF01"));
        return _globalState;
      }, globalState));
