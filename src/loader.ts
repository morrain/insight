...
async function validateSingularMode<T extends object>(
  ...
  return typeof validate === 'function' ? await validate(app) : !!validate
}
...
function createElement(
  ...
  if (strictStyleIsolation) {
    if (!supportShadowDOM) {
      console.warn(
        '[insight]: As current browser not support shadow dom, your strictStyleIsolation configuration will be ignored!'
      )
    } else {
  ...
  if (scopedCSS) {
    const attr = appElement.getAttribute(css.InSightCSSRewriteAttr)
    if (!attr) {
...
function getAppWrapperGetter(
  ...
  return () => {
    if (useLegacyRender) {
      ...
      assertElementExist(
        appWrapper,
        `[insight] Wrapper element for ${appName} with instance ${appInstanceId} is not existed!`
      )
      return appWrapper!
    }
  ...
  const render: ElementRender = ({ element, loading, container }, phase) => {
    if (legacyRender) {
      if (process.env.NODE_ENV === 'development') {
        console.warn(
          '[insight] Custom rendering function is deprecated, you can use the container element setting instead!'
        )
      }
    ...
    if (phase !== 'unmounted') {
      const errorMsg = (() => {
        switch (phase) {
          case 'loading':
          case 'mounting':
            return `[insight] Target container with ${container} not existed while ${appName} ${phase}!`

          case 'mounted':
            return `[insight] Target container with ${container} not existed after ${appName} ${phase}!`

          default:
            return `[insight] Target container with ${container} not existed while ${appName} rendering!`
        }
      })()
...
function getLifecyclesFromExports(scriptExports: LifeCycles<any>, appName: string, global: WindowProxy) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.warn(
      `[insight] lifecycle not found from ${appName} entry exports, fallback to get from window['${appName}']`
    )
  }
  ...
  throw new Error(`[insight] You need to export lifecycle functions in ${appName} entry`)
}
...
export async function loadApp<T extends object>(
  ...
  const markName = `[insight] App ${appInstanceId} Loading`
  if (process.env.NODE_ENV === 'development') {
  ...
  if (await validateSingularMode(singular, app)) {
    await prevAppUnmountedDeferred?.promise
  }
  ...
  let initialAppWrapperElement: HTMLElement | null = createElement(appContent, strictStyleIsolation, scopedCSS, appName)
  ...
  if (sandbox) {
    const sandboxInstance = createSandbox(
      appName,
      // FIXME should use a strict sandbox logic while remount,
      ...
      mount: [
        ...
        async () => {
          if ((await validateSingularMode(singular, app)) && prevAppUnmountedDeferred) {
            return await prevAppUnmountedDeferred.promise
          }
        ...
        async () => {
          if (process.env.NODE_ENV === 'development') {
            const measureName = `[insight] App ${appInstanceId} Loading Consuming`
            performanceMeasure(measureName, markName)
          }
        }
      ],
      unmount: [
        async () => await execHooksChain(toArray(beforeUnmount), app, global),
        async props => unmount({ ...props, container: appWrapperGetter() }),
        unmountSandbox,
        async () => await execHooksChain(toArray(afterUnmount), app, global),
        async () => {
