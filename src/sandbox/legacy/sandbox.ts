...
export default class SingularProxySandbox implements SandBox {
  /** 沙箱期间新增的全局变量 */
  private readonly addedPropsMapInSandbox = new Map<PropertyKey, any>()

  /** 沙箱期间更新的全局变量 */
  private readonly modifiedPropsOriginalValueMapInSandbox = new Map<PropertyKey, any>()

  /** 持续记录更新的(新增和修改的)全局变量的 map，用于在任意时刻做 snapshot */
  private readonly currentUpdatedPropsValueMap = new Map<PropertyKey, any>()
  ...
  inactive() {
    if (process.env.NODE_ENV === 'development') {
      console.info(`[insight:sandbox] ${this.name} modified global properties restore...`, [
  ...
  constructor(name: string) {
    ...
    const proxy = new Proxy(fakeWindow, {
      set(_: Window, p: PropertyKey, value: any): boolean {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[insight] Set window.${p.toString()} while sandbox destroyed or inactive in ${name}!`)
        }
